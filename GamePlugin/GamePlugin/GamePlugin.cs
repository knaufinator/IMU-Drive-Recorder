...
namespace mPUObserver_GamePlugin
{
    public class GamePlugin : IPlugin_Game
    {
        ...
        private const string _gameName = "mPUObserver";
        private const string _processName = "mPUObserver";
            ...
            try
            {
                if (text.StartsWith("S:") & text.EndsWith(":E"))
                {
                 ...
                 //   int gear = (int)double.Parse(strlist[11]);
                    int rpm = (int)double.Parse(strlist[7]);
                    int speed = (int)double.Parse(strlist[8]);
